...
if(isset($_POST['update_product'])){
   ...
   if(!empty($image)){
      if($_FILES['image']["type"] == 'image/jpeg' || $_FILES['image']["type"] == 'image/png'){
         if($image_size > 25000000){
            $message[] = 'image size is too large!';
         }else{
            $update_product = mysqli_query($conn, "UPDATE `products` SET image = '$image' WHERE id = '$update_p_id'") or die('query failed');
            if($update_product){
      ...
      $message[] = "Image file type must be <b>jpeg</b> or <b>png</b>";
      }
   }
